...
def main():
    ...
    meta=json.loads((ROOT/"outputs/phase3w/video_manifest_multi.json").read_text())
    assert sha256(ROOT/"outputs/phase3w/multi.rrd")==meta["rrd_sha256"]
    ...
    old_count=assert_unchanged(json.loads((ROOT/"outputs/phase3w/prior_hashes.json").read_text()))
    ...
    result=dict(status="passed",file=output.relative_to(ROOT).as_posix(),sha256=sha256(output),bytes=output.stat().st_size,
        ...
        source_manifest_sha256=sha256(ROOT/"outputs/phase3w/video_manifest_multi.json"),
