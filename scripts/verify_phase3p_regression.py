"""Compare every non-Brain component with Phase 3V, and protect all prior results."""
import json,hashlib
...
def signature(path):
    ...
    for chunk in RrdReader(path).stream():
        ...
        if p.startswith("/brain") or p in ["/legend","/__properties"]:continue # Recording creation time is metadata, not source data.
...
def main():
    ...
    baseline,_=signature("outputs/phase3v/phase3v_demo.rrd")
    results={}
    for mode in ["representative","t2_population"]:
        ...
        metrics.update(shared_component_streams=len(actual),shared_component_values=sum(len(v) for v in actual.values()),
            nonbrain_components_identical_to_phase3v=True,rrd_sha256=sha256(OUT/(mode+".rrd")))
