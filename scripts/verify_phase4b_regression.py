"""Compare every existing component, including Brain, with Phase 3W, and protect all prior results."""
import json,hashlib
...
def main():
    ...
    baseline,_=signature("outputs/phase4a/demo.rrd")
    results={}
    for mode in ["demo","analysis"]:
        ...
        metrics.update(shared_component_streams=len(actual),shared_component_values=sum(len(v) for v in actual.values()),
            all_existing_components_identical_to_phase4a=True,rrd_sha256=sha256(OUT/(mode+".rrd")))
