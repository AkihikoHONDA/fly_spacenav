...
def main():
    import argparse
    parser=argparse.ArgumentParser();parser.add_argument("--baseline",action="store_true");parser.add_argument("--mode",choices=["representative","t2_population"],default="representative");parser.add_argument("--video",action="store_true");parser.add_argument("--reuse-video",action="store_true");args=parser.parse_args()
    os.chdir(ROOT);out=ROOT/"outputs/phase3p";evidence=ROOT/"docs/evidence_phase3p/memory_probe";evidence.mkdir(parents=True,exist_ok=True)
    ...
    logfile=(out/"memory-probe-viewer.log").open("w")
