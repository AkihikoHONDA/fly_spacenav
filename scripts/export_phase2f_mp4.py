...
def main():
    ...
    meta=json.loads((ROOT/"outputs/phase2f/video_manifest.json").read_text())
    assert sha256(ROOT/"outputs/phase2f/phase2f.rrd")==meta["rrd_sha256"]
    ...
    # Existing captures are every 3 samples plus the last sample. Hold, never interpolate.
    samples=[f["sample"] for f in frames]
    repeat=[samples[i+1]-k if i+1<len(samples) else 1 for i,k in enumerate(samples)]
    assert samples[0]==0 and samples[-1]==421 and sum(repeat)==422
    output=OUT/"phase2f_test00.mp4"
    ...
    (OUT/"decoded_timing.txt").write_text(timing)
    ...
    assert len(records)==422
    assert [int(r[2]) for r in records]==list(range(422))
    ...
    old_count=assert_unchanged(json.loads((OUT/"prior_phase2f_hashes.json").read_text()))
    prior_count=assert_unchanged(json.loads((ROOT/"outputs/phase2f/prior_hashes.json").read_text()))
    version=subprocess.run([ffmpeg,"-version"],capture_output=True,text=True,check=True).stdout
    (OUT/"ffmpeg-version.txt").write_text(version)
    result=dict(status="passed",file=output.relative_to(ROOT).as_posix(),sha256=sha256(output),bytes=output.stat().st_size,
        ...
        source_manifest_sha256=sha256(ROOT/"outputs/phase2f/video_manifest.json"),
        ...
        protected_phase2f_files_unchanged=old_count,protected_prior_files_unchanged=prior_count,
        new_inference=0,new_control_experiments=0,encoder_added_only_in=".tools/video/python")
