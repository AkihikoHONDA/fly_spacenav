...
def main():
    ...
    meta=json.loads((ROOT/"outputs/phase4b/video_manifest_demo.json").read_text())
    assert sha256(ROOT/"outputs/phase4b/demo.rrd")==meta["rrd_sha256"]
    ...
    old_count=assert_unchanged(json.loads((ROOT/"outputs/phase4b/prior_hashes.json").read_text()))
    ...
    write_json(ROOT/"outputs/phase4b/video_manifest_demo.json",meta)
    result=dict(status="passed",file=output.relative_to(ROOT).as_posix(),sha256=sha256(output),bytes=output.stat().st_size,
        ...
        source_manifest_sha256=sha256(ROOT/"outputs/phase4b/video_manifest_demo.json"),
