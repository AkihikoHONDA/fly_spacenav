...
out=Path("outputs/phase4a");m=json.loads((out/"video_manifest_demo.json").read_text())
...
for k,f in enumerate(m["frames"]):
    ...
    if k in [0,im.n_frames//2,im.n_frames-1]:im.convert("RGB").save(f"docs/evidence_phase4a/video_decoded_{k:03d}.png")
