...
def verify(mode):
    out=Path("outputs/phase3v");source=Path("outputs/phase3");scales=json.loads((source/"display_scales.json").read_text())
    ...
    for chunk in RrdReader(out/("phase3v_"+mode+".rrd")).stream():
    ...
    result=dict(status="passed",total_frames=len(expected),entity_frames={p:len(v) for p,v in counters.items()},
        scales_sha256=sha256(source/"display_scales.json"),rrd_sha256=sha256(out/("phase3v_"+mode+".rrd")),
        unique_morphology_count=3,geometry_edges={name:len(branches) for name,(xyz,branches) in geometry.items()},source="unchanged original Phase 3 held-out records and three measured skeletons")
