...
out=Path("outputs/phase3w");m=json.loads((out/"video_manifest_multi.json").read_text())
assert sha256(m["file"])==m["sha256"] and sha256(m["source_record"])==m["source_sha256"]
assert sha256(out/"multi.rrd")==m["rrd_sha256"]
...
for k,f in enumerate(m["frames"]):
    ...
    if k in [0,im.n_frames//2,im.n_frames-1]:im.convert("RGB").save(f"docs/evidence_phase3w/video_decoded_{k:03d}.png")
