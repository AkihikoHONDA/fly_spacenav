...
def main():
    os.chdir(ROOT);out=ROOT/"outputs/phase2";evidence=ROOT/"docs/evidence_phase2";evidence.mkdir(parents=True,exist_ok=True)
    ...
    viewer=subprocess.Popen([binary,str(out/"phase2.rrd"),"--headless","--bind","127.0.0.1","--port","9890","--window-size","1920x1200"],stdout=logfile,stderr=logfile,env=env)
    ...
    try:
        ...
        result=dict(status="passed",renderer="native Rerun 0.37.1 headless, software rasterizer",
            ...
            rrd_sha256=sha256(out/"phase2.rrd"),screenshots=screenshots,
