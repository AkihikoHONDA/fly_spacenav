...
def main():
    ...
    meta=json.loads((ROOT/"outputs/phase3p/video_manifest_t2_population.json").read_text())
    assert sha256(ROOT/"outputs/phase3p/t2_population.rrd")==meta["rrd_sha256"]
    ...
    old_count=assert_unchanged(json.loads((ROOT/"outputs/phase3p/prior_hashes.json").read_text()))
    ...
    result=dict(status="passed",file=output.relative_to(ROOT).as_posix(),sha256=sha256(output),bytes=output.stat().st_size,
        ...
        source_manifest_sha256=sha256(ROOT/"outputs/phase3p/video_manifest_t2_population.json"),
