...
def main():
    out=Path("outputs/phase2");cfg=load_config();disp=cfg["display"]
    ...
    for m in manifest["episodes"]:
        ...
        for k,t in enumerate(d["observation_time"]):
            ...
            expected[ns]=dict(id=m["id"],image=np.rint(d["images"][k]*255).astype(np.uint8).reshape(-1),
                ...
                shape=rms_colors(rms[k],disp["rms_upper"]),position=s[:2]*[1,-1],
                velocity=s[2:]*[1,-1]*disp["velocity_arrow_seconds"],
                acceleration=d["u_applied"][k]*[1,-1]*disp["acceleration_arrow_seconds_squared"],
                applied=d["u_applied"][k],error=np.linalg.norm(s[:2]-[10,0]),speed=np.linalg.norm(s[2:]))
    counters={key:set() for key in ["image","grid","receptors","shape","position","velocity","acceleration","ax","ay","error","speed"]}
    for c in RrdReader(out/"phase2.rrd").stream():
        ...
        mapping={"/input/image":("image","Image:buffer"),"/grid/hex":("grid","Points2D:colors"),
          ...
          "/orbit/craft":("position","Points2D:positions"),"/orbit/velocity":("velocity","Arrows2D:vectors"),
        ...
        for row,ns in enumerate(b.column("physical_display_s").cast("int64").to_pylist()):
            ...
            elif key in ["position","velocity","acceleration"]:np.testing.assert_allclose(value,[e[key]],atol=2e-6,rtol=1e-6)
            else:
                assert e["id"] in p
    ...
    result=dict(status="passed",samples_per_panel={k:len(v) for k,v in counters.items()},
        geometry_edges=len(edges),rrd_sha256=sha256(out/"phase2.rrd"),source="each replay's own images, full neural responses and applied control")
