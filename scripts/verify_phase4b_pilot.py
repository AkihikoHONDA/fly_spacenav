...
OUT=Path('outputs/phase4b');ASSETS=Path('assets/fly_pilot_phase4b')
...
def rows(path):
 ...
 return result
def verify(mode):
 path=OUT/(mode+'.rrd');data=rows(path);old=rows(Path('outputs/phase4a')/(mode+'.rrd'))
 ...
 for key,values in old.items():
  if key[0] in unchanged:assert data[key]==values,key
 ...
 result=dict(status='passed',samples=counts,feet_verified=counts*2,max_fk_contact_error=maximum,minimum_adjacent_bend_dot=minimum_dot,static_assets_verified=len(asset_paths),joystick_components_identical_to_phase4a=True,rrd_sha256=sha256(path))
 return result
