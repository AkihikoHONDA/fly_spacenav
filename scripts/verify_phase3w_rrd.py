...
def verify(mode):
    out=Path("outputs/phase3w");source=Path("outputs/phase3");scales=json.loads((source/"display_scales.json").read_text())
    ...
    multi=mode!="representative"
    from flyrendezvous.multi_phase3w import load_groups,load_context
    groups=load_groups() if multi else {}
    visual=manifest.get("display_parameters",{})
    ...
    meshes={name:(vertices,faces,normals) for name,vertices,faces,normals in (load_context() if multi else load_meshes())}
    ...
    for chunk in RrdReader(out/(mode+".rrd")).stream():
        ...
        if chunk.is_static:
            ...
            if p.startswith("/brain/context/"):
                ...
                rgba=visual["right_context_rgba" if name in ["ME_R","LO_R","LOP_R"] else "context_rgba"] if multi else [42,49,62,255]
                np.testing.assert_array_equal(unpack(b.column("LineStrips3D:colors")[0].as_py()),[rgba[:3]])
                assert int(b.column("LineStrips3D:colors")[0].as_py()[0])&255==rgba[3]
                static_seen.add(p)
            if p.startswith("/brain/multi/") and p.endswith("/arbors"):
                name=p.split("/")[3];x,e,provenance=groups[name]
                np.testing.assert_array_equal(b.column("LineStrips3D:strips")[0].as_py(),x[e].astype(np.float32))
                assert provenance==json.loads((out/"provenance.json").read_text())[name]["roots"]
                static_seen.add(p)
            continue
        col=None
        ...
        elif p.startswith("/brain/multi/"):col="LineStrips3D:colors" if p.endswith("/arbors") else "Points3D:labels"
        ...
        for row,ns in enumerate(b.column("physical_display_s").cast("int64").to_pylist()):
            ...
            if ns not in expected:
                if p.startswith("/brain/multi/"):assert p.endswith("/arbors") and v==[0]
            ...
            elif p.startswith("/brain/multi/"):
                name=p.split("/")[3];j=TYPES.index(name);x,edges,provenance=groups[name]
                if p.endswith("/arbors"):
                    np.testing.assert_array_equal(unpack(v),intensity_color(name,e["q"][j])[None])
                    assert len(v)==1 and (int(v[0])&255)==visual["morphology_alpha"]
                else:
                    assert v==[f"{name} x12  q {e['q'][j]:.2f}"]
                    idx={"T2":np.argmax(x[:,0]),"T4a":np.argmin(x[:,2]),"T5d":np.argmax(x[:,2])}[name]
                    np.testing.assert_array_equal(b.column("Points3D:positions")[row].as_py(),x[[idx]].astype(np.float32))
    ...
    assert {"/brain/context/"+name for name in meshes}<=static_seen
    if multi:assert {"/brain/multi/"+name+"/arbors" for name in groups}<=static_seen
    result=dict(status="passed",total_frames=len(expected),entity_frames={p:len(v) for p,v in counters.items()},
        ...
        unique_morphology_count=36 if multi else 3,context_mesh_count=len(meshes),geometry_edges={name:len(e) for name,(x,e,p) in groups.items()} if multi else {name:len(b) for name,(x,b) in geometry.items()},source="original Phase 3 held-out records and pinned measured geometry")
    write_json(out/("rrd_audit_"+mode+".json"),result);print(json.dumps(result,indent=2))
if __name__=="__main__":
    for mode in ["representative","multi","analysis"]:verify(mode)
