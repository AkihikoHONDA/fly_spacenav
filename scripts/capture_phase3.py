...
def main():
    import argparse
    parser=argparse.ArgumentParser();parser.add_argument("--mode",choices=["demo","analysis"],default="demo");parser.add_argument("--video",action="store_true");parser.add_argument("--reuse-video",action="store_true");args=parser.parse_args()
    os.chdir(ROOT);out=ROOT/"outputs/phase3";evidence=ROOT/"docs/evidence_phase3";evidence.mkdir(parents=True,exist_ok=True)
    ...
    viewer=subprocess.Popen([binary,str(out/("phase3_"+args.mode+".rrd")),"--headless","--bind","127.0.0.1","--port","9890","--window-size","2240x1400"],stdout=logfile,stderr=logfile,env=env)
    ...
    try:
        ...
        initial=state()
        manifest=json.loads((out/("viewer_"+args.mode+".json")).read_text())
        ...
        screenshots=[]
        for label,k in zip(["initial","early","diagonal","braking","hold"],indices):
            ...
            path=evidence/(args.mode+"_"+label+".png")
        ...
        result=dict(status="passed",renderer="native Rerun 0.37.1 headless, software rasterizer",
            ...
            rrd_sha256=sha256(out/("phase3_"+args.mode+".rrd")),screenshots=screenshots,
        ...
        if args.video:
            ...
            if args.reuse_video:
                previous=json.loads((out/"video_manifest.json").read_text())
                video=ROOT/previous["file"];assert sha256(video)==previous["sha256"]
                assert previous["rrd_sha256"]==sha256(out/("phase3_"+args.mode+".rrd"))
                assert previous["source_sha256"]==entry["record_sha256"]
                frame_meta=previous["frames"];durations=previous["durations_ms"]
            else:
                frames_dir=out/"video_frames";frames_dir.mkdir(exist_ok=True)
                frame_meta=[];images=[]
                # Every third recorded sample, plus the actual last sample; no interpolation.
            ...
            live=[];deadline=time.monotonic()+25
            while time.monotonic()<deadline:
                ...
                if before>=round(d["observation_time"][-1]*1e9):
                    ...
                    before=current(state())
                path=evidence/f"live_{len(live):02d}.png"
                tool("screenshot",{"save_path":str(path),"pixels_per_point":1})
            ...
            tool("set_time",{"timeline":"physical_display_s","time":round(d["observation_time"][-1]*1e9),"play":False})
            write_json(out/"video_manifest.json",dict(file=video.relative_to(ROOT).as_posix(),sha256=sha256(video),
                ...
                source_record=entry["record"],source_sha256=entry["record_sha256"],rrd_sha256=sha256(out/("phase3_"+args.mode+".rrd")),
        ...
        write_json(evidence/("screenshots_"+args.mode+".json"),result)
