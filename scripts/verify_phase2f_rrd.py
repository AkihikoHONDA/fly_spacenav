...
def main():
    out=Path("outputs/phase2f");scales=json.loads((out/"display_scales.json").read_text())
    ...
    sources={m["id"]:m for m in json.loads(Path("outputs/phase2/replays.json").read_text())}
    for m in manifest["episodes"]:
        ...
        indices=[np.flatnonzero(d["cell_type"]==n) for n in TYPES]
        pos=[lattice_xy(d["u"][idx],d["v"][idx]) for idx in indices]
        for k,t in enumerate(d["observation_time"]):
            delta=np.stack([d["activity"][k,idx].astype(float)-d["baseline"][idx] for idx in indices])
    ...
    for chunk in RrdReader(out/"phase2f.rrd").stream():
        ...
        for row,ns in enumerate(b.column("physical_display_s").cast("int64").to_pylist()):
            ...
            if ns not in expected:
                assert p=="/readout" and v[0].startswith("# RESET");continue
            e=expected[ns];assert ns not in seen;seen.add(ns)
            if p=="/input/image":np.testing.assert_array_equal(v[0],e["image"])
            elif p=="/readout":
                assert f"Sample {e['sample']};" in v[0]
                assert f"T4a RMS **{e['raw'][2]:.5f}**; q **{e['q'][2]:.3f}**" in v[0]
                assert f"{e['physical_time']:.1f} physical s" in v[0]
            elif p=="/bars/fill":
                ...
                labels=[f"{n}   q {e['q'][j]:.3f}   RMS {e['raw'][j]:.5f}" for j,n in enumerate(TYPES)]
                assert v==labels
                ...
                _,_,kind,ident,name=p.split("/")
                assert ident==e["id"];j=TYPES.index(name)
                np.testing.assert_allclose(v,[e[kind][j]],atol=1e-12)
            else:
    ...
    assert len(counters)==35,len(counters) # 20 episode-specific series plus 15 shared entities
    result=dict(status="passed",total_frames=len(expected),entity_frames={p:len(v) for p,v in counters.items()},
        scales_sha256=sha256(out/"display_scales.json"),rrd_sha256=sha256(out/"phase2f.rrd"),
