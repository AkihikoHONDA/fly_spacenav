...
def main():
    import argparse
    parser=argparse.ArgumentParser();parser.add_argument("--video",action="store_true");parser.add_argument("--reuse-video",action="store_true");args=parser.parse_args()
    os.chdir(ROOT);out=ROOT/"outputs/phase2f";evidence=ROOT/"docs/evidence_phase2f";evidence.mkdir(parents=True,exist_ok=True)
    ...
    viewer=subprocess.Popen([binary,str(out/"phase2f.rrd"),"--headless","--bind","127.0.0.1","--port","9890","--window-size","2240x1400"],stdout=logfile,stderr=logfile,env=env)
    ...
    try:
        ...
        result=dict(status="passed",renderer="native Rerun 0.37.1 headless, software rasterizer",
            ...
            rrd_sha256=sha256(out/"phase2f.rrd"),screenshots=screenshots,
        ...
        if args.video:
            ...
            if args.reuse_video:
                ...
                assert previous["rrd_sha256"]==sha256(out/"phase2f.rrd")
                ...
                video_indices=sorted(set(list(range(0,len(d["sample_id"]),3))+[len(d["sample_id"])-1]))
            ...
            write_json(out/"video_manifest.json",dict(file=video.relative_to(ROOT).as_posix(),sha256=sha256(video),
                format="animated WebP, native Rerun renders; no video framework added",
                duration_ms=sum(durations),durations_ms=durations,frames=frame_meta,playback_speed=15,
                source_record=entry["record"],source_sha256=entry["record_sha256"],rrd_sha256=sha256(out/"phase2f.rrd"),
                continuous_playback_frames=live,mp4_created=False,
                mp4_reason="ffmpeg/ffprobe and video codecs are absent; existing Pillow animated WebP used instead"))
