"""Phase 5B truth rollout; Phase 3 endpoint/safety and image-only policy semantics retained."""
import json,time
...
def rollout(cfg,case,mode,adapter=None,pooling=None,policy=None,full=False,max_steps=None,condition=None):
    from . import physics_phase5b as physics
    truth_cfg=json.loads(Path('configs/phase5b/truth_model.json').read_text())
    if condition is None or condition['name']=='H0':raise ValueError('Truth condition required')
    ...
    absolute=physics.to_inertial(s,physics.chief_initial(truth_cfg))
    absolute_states=[absolute.copy()];force_rows=[];eclipse_rows=[]
    ...
    for k in range(steps):
        ...
        f=physics.forces(k*dt,absolute,applied,condition,truth_cfg)
        force_rows.append([[*f[body][term]] for body in ['target','chaser'] for term in ['two_body','j2','srp','drag','residual','control']])
        eclipse_rows.append([physics.shadow(absolute[:2],truth_cfg),physics.shadow(absolute[4:6],truth_cfg)])
        absolute=physics.advance(k*dt,absolute,applied,condition,truth_cfg)
        absolute_states.append(absolute.copy())
        next_s=physics.to_relative(absolute)
    ...
    arrays["absolute_states_ECI"]=np.asarray(absolute_states)
    arrays["forces_ECI_m_s2"]=np.asarray(force_rows)
    arrays["force_terms"]=np.array(['two_body','j2','srp','drag','residual','control'])
    arrays["force_bodies"]=np.array(['target','chaser'])
    arrays["eclipse"]=np.asarray(eclipse_rows)
