...
from .population_phase3p import load_population
...
def build(mode):
    ...
    population = load_population() if mode=="t2_population" else None
    if population is not None:representatives.pop("T2")
    brain_eye=bp.archetypes.EyeControls3D(position=[880,0,180] if population is not None else [810,86,183],look_target=[750,278,161] if population is not None else [708,298,180],eye_up=[0,0,1])
    ...
    brain=bp.Spatial3DView(origin="/brain",contents=["/brain/context/ME_R/**","/brain/context/LO_R/**","/brain/context/LOP_R/**","/brain/representatives/**","/brain/populations/**"],
    ...
    if mode in ["representative","t2_population"]:
        layout=bp.Vertical(
    ...
    rr.init("FlyRendezvous-Phase3P-"+mode,spawn=False,strict=True)
    ...
    if population is not None:
        xyz_p,edges_p,provenance=population
        # Full source geometry once, 1 shared temporal color. Avoid 725*777 entity updates.
        rr.log("/brain/populations/T2/arbors",rr.LineStrips3D(xyz_p[edges_p],radii=rr.Radius.ui_points(.20)),static=True)
        write_json(OUT/"population_provenance.json",dict(entity="/brain/populations/T2/arbors",index_convention="half-open offsets; edge row = rendered strip index",roots=provenance,vertices=len(xyz_p),edges=len(edges_p)))
    ...
    rr.log("/legend",rr.TextDocument(
        ("Measured right T2 population (725) · All T2 share one type response · " if population is not None else "Representative morphologies · Color intensity = within-type relative response · ") +
    ...
    for m in chosen:
        ...
        for k,t in enumerate(d["observation_time"]):
            ...
            if population is not None:
                rgb=intensity_color("T2",q[k,0])
                rr.log("/brain/populations/T2/arbors",rr.LineStrips3D.from_fields(colors=[*rgb,160]))
                rr.log("/brain/populations/T2/label",rr.Points3D([xyz_p[np.argmax(xyz_p[:,2])]],radii=rr.Radius.ui_points(0),labels=[f"T2 x725  q {q[k,0]:.2f}"],colors=PALETTE["T2"]))
        ...
        if population is not None:
            rr.log("/brain/populations/T2/arbors",rr.LineStrips3D.from_fields(colors=[0,0,0,0]))
            rr.log("/brain/populations/T2/label",rr.Clear(recursive=True))
    ...
    write_json(OUT/("viewer_"+mode+".json"),dict(mode=mode,episodes=manifest,
        ...
        source="original once-only held-out learner records",morphology_count=727 if population is not None else 3,
        timeline="physical_display_s",playback_speed=15))
    rr.disconnect()
    print("Saved",mode,flush=True)
if __name__=="__main__":
    import argparse
    p=argparse.ArgumentParser();p.add_argument("--mode",choices=["representative","t2_population","both"],default="both");args=p.parse_args()
    for mode in (["representative","t2_population"] if args.mode=="both" else [args.mode]):build(mode)
