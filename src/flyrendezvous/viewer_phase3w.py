...
def build(mode):
    ...
    if mode in ["representative","multi"]:
        layout=bp.Vertical(
        ...
        layout=bp.Vertical(
            bp.Horizontal(brain,closeup,column_shares=[1.25,1]),
    ...
    rr.init("FlyRendezvous-Phase3W-"+mode,spawn=False,strict=True)
    ...
    write_json(OUT/("viewer_"+mode+".json"),dict(mode=mode,episodes=manifest,
        ...
        source="original once-only held-out learner records",morphology_count=36 if multi else 3,display_parameters=visual if multi else "Phase 3V unchanged",
        timeline="physical_display_s",playback_speed=15))
    rr.disconnect()
    print("Saved",mode,flush=True)
if __name__=="__main__":
    import argparse
    p=argparse.ArgumentParser();p.add_argument("--mode",choices=["representative","multi","analysis","both"],default="both");args=p.parse_args()
    for mode in (["representative","multi"] if args.mode=="both" else [args.mode]):build(mode)
