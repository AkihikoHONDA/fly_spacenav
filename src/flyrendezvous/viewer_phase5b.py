...
CONDITION=os.environ["PHASE5B_CONDITION"]
assert CONDITION in ["H0","E0","RM3"]
OUT=Path("outputs/phase5b/demo")/CONDITION
OUT.mkdir(parents=True,exist_ok=True)
SOURCE=Path("outputs/phase5b")
load_config=lambda:json.loads(Path("configs/phase5b/training.json").read_text())
...
def build(mode,allow_missing=False):
    ...
    assert sha256(lock["readout"])==lock["readout_sha256"]==testlock["readout_sha256"]
    assert sha256(SOURCE/"display_scales.json")==lock["display_scales_sha256"] and not scales["test_used"]
    representatives=load_representatives()
    multi=True
    from .pilot_phase4c import PILOT_EYE,PILOT_LOOK,PILOT_UP,reset as pilot_reset,load_asset,log_static as pilot_static,log_sample as pilot_sample
    ...
    pilot=bp.Spatial3DView(origin="/pilot",name="Fly Pilot | illustrative command",background=[14,18,26],line_grid=False,
        eye_controls=bp.archetypes.EyeControls3D(position=PILOT_EYE,look_target=PILOT_LOOK,eye_up=PILOT_UP))
    pilot_values=bp.TextDocumentView(origin="/pilot_values",name="Presentation layer")
    guide=bp.Spatial2DView(origin="/pilot_guide",name="Command XY")
    pilot_panel=bp.Vertical(pilot,bp.Horizontal(guide,pilot_values,column_shares=[1,3.6]),row_shares=[2.8,1.4])
    ...
    rr.init("FlyRendezvous-Phase5B-"+CONDITION+"-"+mode,spawn=False,strict=True)
    ...
    chosen=[json.loads((SOURCE/"evaluation"/CONDITION/"learner"/"test_00.json").read_text())]
    manifest=[];offset=0.
    for m in chosen:
        ...
        for k,t in enumerate(d["observation_time"]):
            ...
            rr.log("/status",rr.TextDocument(
                f"Truth condition: {CONDITION} · Disturbance: {'ON' if CONDITION=='RM3' and 120<=t<180 else 'OFF'}\n\n**{m['id']} · {t:.1f} s · {phase(s,a,t,goal)}**\n\n"
