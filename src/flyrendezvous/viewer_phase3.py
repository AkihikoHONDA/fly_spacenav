...
def build(mode):
    ...
    assert sha256(OUT/"readout.npz")==lock["readout_sha256"]==testlock["readout_sha256"]
    ...
    eye=bp.archetypes.EyeControls3D(position=(center+[0,-155,0]).tolist(),look_target=center.tolist(),eye_up=[0,0,1])
    context_eye=bp.archetypes.EyeControls3D(position=[524,262,-800],look_target=[524,262,167],eye_up=[0,-1,0])
    ...
    close=bp.Spatial3DView(origin="/brain",contents=["/brain/representative"],name="T4a | fixed relative color",
        background=[15,20,30],line_grid=False,eye_controls=eye)
    context=bp.Spatial3DView(origin="/brain",name="FAFB14.1 context [um]",
        background=[15,20,30],line_grid=False,eye_controls=context_eye)
    ...
    if mode=="demo":
        layout=bp.Horizontal(
            bp.Vertical(orbit,status,row_shares=[4,1]),
            bp.Vertical(bp.Horizontal(sensor,bars,column_shares=[1,1.2]),
                        bp.Horizontal(context,close),legend,row_shares=[1.4,1.4,.7]),
            bp.Vertical(maps["T2"],maps["T4a"],maps["T5d"]),
            column_shares=[.43,.36,.21])
    else:
        absolute=bp.Spatial3DView(origin="/absolute",name="T4a | absolute RMS 0..0.22",
            background=[15,20,30],line_grid=False,eye_controls=eye)
        layout=bp.Vertical(
            bp.Horizontal(orbit,sensor,bars,absolute,close,column_shares=[1.3,.8,1.3,1,1]),
            bp.Horizontal(*maps.values()),
            bp.Horizontal(bp.TimeSeriesView(origin="/series/raw",name="Raw RMS"),
                          bp.TimeSeriesView(origin="/series/q",name="Fixed q",axis_y=bp.ScalarAxis(range=[0,1],zoom_lock=True)),
                          bp.TimeSeriesView(origin="/series/acceleration",name="Applied a [m/s^2]"),
                          bp.TimeSeriesView(origin="/series/error",name="Goal error [m]"),
                          bp.TimeSeriesView(origin="/series/speed",name="Speed [m/s]")),
            bp.Horizontal(status,legend,bp.TextDocumentView(origin="/timing",name="Sample / physical / neural timing")),
            row_shares=[1.2,1.1,.8,.7])
    rr.init("FlyRendezvous-Phase3-"+mode,spawn=False,strict=True)
    rr.save(str(OUT/("phase3_"+mode+".rrd")))
    ...
    rr.log("/legend",rr.TextDocument(
        "T4a: **721 model cells → one measured arbor**. Same arbor in context and close-up.\n\n"
        "No cell-wise anatomical correspondence or branch propagation. Grey anatomy: unassigned.\n\n"
        "**q = fixed non-test p05..p95**, clipped to [0,1]. Raw RMS retained. No absolute comparison across types.\n\n"
        "Maps: blue negative / white zero / red positive; fixed type-specific limits. 45,669 cells computed; 57 types read out.",
        media_type="text/markdown"),static=True)
    results=json.loads((OUT/"evaluation.json").read_text())["learner"]
    ...
    for m in chosen:
        ...
        for k,t in enumerate(d["observation_time"]):
            ...
            rr.log("/brain/representative",rr.LineStrips3D(xyz[edges],colors=rms_colors(q[k,2],1),radii=rr.Radius.ui_points(1.6)))
            rr.log("/absolute/arbor",rr.LineStrips3D(xyz[edges],colors=rms_colors(raw[k,2],.22),radii=rr.Radius.ui_points(1.6)))
            ...
            rr.log("/status",rr.TextDocument(
                ...
                f"Center **{distance:.2f} m** · surface clearance **{distance-1:.2f} m** · goal error **{error:.3f} m**\n\n"
                f"Speed **{speed:.4f} m/s** · LVLH a **({a[0]:+.5f}, {a[1]:+.5f}) m/s²**\n\n"
                f"Recorded outcome: **{m['termination_reason']}** at {m['final_time_s']:.1f} s. "
                "Vectors: v ×20 s / applied a ×400 s².",media_type="text/markdown"))
        ...
        for path in ["/input","/maps","/bars/fill","/bars/labels","/brain/representative","/absolute","/orbit/trail","/orbit/craft","/orbit/velocity","/orbit/acceleration"]:
    ...
    write_json(OUT/("viewer_"+mode+".json"),dict(mode=mode,episodes=manifest,
        scales_sha256=sha256(OUT/"display_scales.json"),new_inference=False,
        source="original once-only held-out learner records",morphology_count=1,
