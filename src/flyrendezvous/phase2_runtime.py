...
from .hcw import Teacher
from .recording import write_json,sha256

def load_config():return json.loads(Path("configs/phase2.json").read_text())
def adapter_config():
...
def make_splits(cfg):
    ...
    for split,spec in cfg["splits"].items():
        ...
        for i in range(spec["count"]):
            ...
            bounds=np.array(cfg[category+"_bounds"])
            state=rng.uniform(bounds[:,0],bounds[:,1])
...
def safety(state,camera,cfg):
    ...
    if state[0]<=cfg["x_bounds_m"][0] or state[0]>=cfg["x_bounds_m"][1]:return "range_exit"
...
def rollout(cfg,case,mode,adapter=None,pooling=None,policy=None,full=False,max_steps=None):
    ...
    data={k:[] for k in ["images","projection","u_raw","u_applied","u_teacher_raw","u_teacher",
                         "control_mask","phi","type_mean","type_rms","receptor_input","activity","wall_seconds"]}
