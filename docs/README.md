...
案内文書、Phase報告書、設計記録、出典・対応表をGit管理対象にしています。確定した小容量の`models/**/readout.npz`も管理します。学習途中の候補モデル、ローカル導入ツール（`.tools/`）、取得資産、数値ログ、RRD、動画、証拠画像、元の作業指令などは別管理です。報告書中のそれらへのリンクは、該当ファイルを持つ作業環境で参照してください。

今回のREADME再編では説明文書と文書用のGit除外設定だけを更新しています。制御器、readout、物理モデル、カメラ、既存test結果は変更していません。

Git管理済みファイルは`.gitignore`への追加だけでは除外されません。今回の整理では`git rm --cached`でツールと候補モデルをindexから外し、手元のファイルは保持しました。2026-09-14の履歴整理では、これらをmainの過去のcommitからも除去しました。確定readoutとローカル実ファイルは保持し、変更前の履歴はGit除外された`outputs/git-history-cleanup-*/before.bundle`へバックアップしています。
